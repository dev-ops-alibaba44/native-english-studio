...
**(a) + (b) Dynamic seat numbering.** Multiple unused seats of the same type looked identical in both the `/agency/students/new` dropdown and the "尚未分配的席次" list on `/agency/students` — no way to tell "Standard seat" from "Standard seat" apart. Added `numberSeatsByType()` in `lib/seats.ts`: numbers each seat within its own type (Standard #1, #2, … / Premium #1, #2, … counted separately), computed fresh at render time from purchase order (oldest first) rather than stored anywhere — so numbers stay correct automatically as seats get added, assigned, or canceled, with no stale numbers left behind. Both pages now show e.g. "標準席次 #2 · 2026–2027".

**(c) "已註冊學生" section title** added on `/agency/students` directly above the sort buttons and student list, same `font-display font-bold text-base` style as "尚未分配的席次（5）" above it, with a live count.

**(d) Confirmation pop-up before submitting the sign-up form.** New `components/ConfirmSubmitButton.tsx` — a plain submit button that shows a native browser `confirm()` dialog (a real pop-up with OK/Cancel) before letting the form go through, restating that the four identity fields lock immediately. Kept intentionally simple (native browser dialog, not a custom styled modal) so the page around it could stay a server component — happy to swap in a fully custom-styled modal instead if you'd prefer that over the native one.

**(g) The actual bug behind "no screen to set a password."** The invite email's link was arriving at `/auth/callback`, a server Route Handler — but Supabase's invite links (as generated by `inviteUserByEmail` on this project) return the session as `#access_token=...` in the URL *fragment*, not as a `?code=` query param. Fragments are never sent to the server, by design, in every browser — so `/auth/callback` had no way to see the token no matter what redirect target it was given, which is exactly the stray `#access_token=...` you saw stuck on the `/login?error=auth_callback_failed` URL. Fixed by pointing the invite link straight at `/auth/set-password` (a client page) and having it parse `window.location.hash` directly in the browser, then call `supabase.auth.setSession()` before showing the password form. Also added a clear "連結已失效" state for expired/already-used links instead of a silent failure.

**(h) Branded invite email.** The default Supabase email (`noreply@mail.app.supabase.io`, plain black-and-white template) is a Supabase Dashboard setting, not application code — nothing in the zip can override it directly. Two manual steps get you the rest of the way:
1. **Custom SMTP** (so it sends from `info@nativeenglish.ca` instead of Supabase's shared address): Supabase Dashboard → Project Settings → Auth → SMTP Settings → enable custom SMTP, host `smtp.gmail.com`, port `587`, username `info@nativeenglish.ca`, password = a Google Workspace **App Password** (Google Account → Security → 2-Step Verification → App passwords — a regular Gmail password won't work for SMTP), sender name "Native English Studio", sender email `info@nativeenglish.ca`.
2. **Branded template**: `supabase/email-templates/invite-user.html` in this zip — cream background, navy heading, your logo, Traditional Chinese copy, matching the app's login page styling. Paste its contents into Supabase Dashboard → Authentication → Email Templates → Invite user → Message body → Save.

**(i) Seat-assignment-failure safety net, baked into the app as requested.**
- New `profiles.pending_seat_deadline` column (`batch24_pending_seat_cleanup.sql`).
- If the seat-assignment step fails after the account and profile already exist (e.g. a race where two staff pick the same seat at once — now guarded against directly, but the fallback matters regardless of cause), `createStudentAccount` sets this to 7 days out and redirects to `/agency/students` with a specific warning banner, instead of the old generic error. A persistent per-student warning also shows on their card on every visit until it's resolved: "⚠️ 尚未成功指派席次，此帳號將於 {date} 自動刪除。"
- Assigning any seat to that student (the normal inline dropdown already on the page) clears the flag automatically.
- New `app/api/cron/cleanup-pending-students/route.ts` + `vercel.json` — runs daily (18:00 UTC / 02:00 Taipei), finds any student past their deadline still with no seat, double-checks for a seat one more time as insurance, then deletes the account (cascades via the existing FK, no separate cleanup needed). Protected by a `CRON_SECRET` env var that Vercel automatically sends back as a bearer token on scheduled calls — **set `CRON_SECRET` in Vercel's project environment variables** (any random string, e.g. `openssl rand -hex 32`) for this to actually run; the route refuses to do anything without it.

**(e) and (f)** — confirmed working as reported, no changes needed.

**How to apply:**
1. Run `supabase/batch24_pending_seat_cleanup.sql`.
2. Stop the dev server, unzip, `rm -rf node_modules .next package-lock.json && npm install && npm run build`.
3. In Supabase Dashboard: set up Custom SMTP and paste the branded template, per (h) above.
4. In Vercel project settings → Environment Variables: add `CRON_SECRET` (any random string) — needed for (i)'s cleanup job to run at all. `vercel.json`'s cron schedule takes effect automatically on the next deploy.
5. Push.
...
- **Seat expiry is now derived live from `admission_cycle_end_year` instead of trusting the separately-stored `expires_at` column.** Dan found a real case where a seat's displayed expiry didn't match its admission cycle (e.g. a 2027–2028 cycle seat still showing a 2027 expiry date). Reviewed the insert/update code path by path and couldn't find the exact defect through static review alone — rather than guess further, `lib/seats.ts` now exports `effectiveExpiresAt()`, which always recomputes August 31 of the cycle end year when a cycle is set, and only falls back to the stored column for legacy seats with no cycle at all. Both `assertSeatActive()` and the billing page's display now go through this function, so the cycle is the one source of truth and this class of drift can't recur regardless of which write path caused the original mismatch.
- Confirmed: the seats query on the billing page has no status filter — every seat status maps into one of the three grouped sections, so nothing should be silently hidden. If seats still seem to be missing after this, worth checking the `seats` table directly in Supabase's Table Editor for that agency's `agency_id` before assuming it's a display bug.

**Next batch, isolated, nothing else bundled in:** splitting the license into its own Stripe subscription so it renews independently at $2,000/yr, with seats moved to independently-dated charges — the fix for Stripe's cancellation screen showing one combined $3,825 figure. Real charging-logic surgery, kept separate from everything else so it can be tested carefully on its own.

## 🆕 Batch 20 — admission-cycle-based seat expiry, license-lapse gate, fix Batch 19's build error

**Fixed a build error in Batch 19** (`app/actions/student-identity.ts` and the identity page): two `.select()` calls were written as string *concatenation* (`"a, b, " + "c, d"`) across multiple lines. Supabase's TypeScript client infers field types from `.select()` strings using literal-type parsing, which only works on an actual string literal — concatenating two strings with `+` widens the type to plain `string` and breaks that inference, producing the `GenericStringError` type Dan hit locally. Both calls rewritten as single literals; checked every other `.select()` call in the project for the same pattern (none found).

**The real fix Dan asked for — closing the "cancel the license, keep coasting on seats bought last cycle" hole:**

- **Seats now expire on a real calendar boundary, not "365 days from purchase."** Every seat has an `admission_cycle_end_year` — the year the student starts university. A seat for the 2026–2027 cycle (student starting September 2027) expires August 31, 2027, full stop, regardless of when it was actually purchased. Selected via a required dropdown on the initial checkout form and the "Add seats" form (one cycle per submission — add seats separately if different seats need different cycles). Legacy seats (bought before this batch) keep their old 365-day-based `expires_at` until an admin sets a cycle for them via a new inline "設定入學年度" control that appears on any unused seat missing one.
- **The agency's license status is now checked independently of any individual seat's status**, in `assertSeatActive()`. If `agencies.plan_status` isn't `'active'` — canceled, past due, never subscribed — every student under that agency is blocked immediately, even ones whose own seat's admission-cycle date hasn't arrived yet. This is what actually closes the gaming hole: there's no scenario where a lapsed license still leaves usable seats.
- Both rules are stated in the billing page's rules block, and the seat list now shows each seat's admission cycle next to its expiry date.

**Not done in this batch** — the original cosmetic complaint that started this (Stripe's cancellation-preview screen showing one combined $3,825/yr figure instead of the license's $2,000 alone) is still open. Splitting the license into its own Stripe Subscription and seats into independent one-time charges with a scheduled renewal job would fix that display, but the actual risk Dan was worried about (leftover seats usable after a lapsed license) is now closed by the two changes above without needing that bigger rewrite. Worth revisiting only if the combined-total display itself is still a problem, separately from the security concern.

## 🆕 Batch 19 — seat list regrouping, upgrade forfeits cancel window, locked student identity fields, Stripe locale fix

**No SQL changes carried over from a broken build this time** — Batch 18.1 built successfully. This batch is additive.

- **Seat list reorganized into three groups**, per Dan: 新增、尚未使用的席次 (unused) / 使用中的席次 (active) / 已封存・已到期・已取消的席次 (archived+expired+canceled combined). Every assigned seat now always shows the student's real display name inline, not in small secondary text.
- **Upgrading a seat now permanently forfeits its 7-day cancel/refund window**, even if it's technically still unused and still inside the original 7 days. Important honesty note: Stripe trials only apply at the whole-subscription level, not per line item, so there's no way to literally "end the trial for just one seat." What this actually does — and what solves the real problem Dan described (a canceled seat that had already been prorated mid-upgrade creating a confusing reversal) — is force the seat's status to `active` the moment it's upgraded, which is what `cancelSeat` checks against. The upgrade confirmation screen now states this plainly before the admin confirms.
- **Student identity fields** (`supabase/batch19_student_identity.sql`, `app/actions/student-identity.ts`, `/agency/students/[id]/identity`): birthdate, Chinese name, legal English first/last name — each locks permanently after being set once, enforced server-side (not just a disabled UI input). Preferred/screen name has no lock but is throttled to one change per rolling 30 days. A warning banner states the once-only rule before the admin can submit, and the button reads "我已確認無誤，儲存" rather than a plain "Save."
- **Stripe Checkout and the Billing Portal now explicitly request `locale: "zh-TW"`** — Dan's test browser was showing English because neither call specified a locale before, so Stripe fell back to browser/account defaults.

**Not code — Stripe Dashboard settings, not touched by this zip:**
- **Disabling "Link"** as a payment option: Settings → Payment methods → find Link → toggle off. Takes a few minutes to apply.

**Explained, not changed in code this batch (see chat for full reasoning):**
- **Why the subscription renews at $3,825 instead of $2,000**: this is standard Stripe behavior when the license and all seats live on one shared Subscription object — everything on one subscription renews together, on the same date, for the combined total. It is *not* a double-charge, and clicking "Cancel subscription" does not charge anything — it only stops future renewals. But it doesn't match Dan's stated rule that each seat has its own independent 365-day life. The correct fix is a real architecture change — the license becomes its own subscription, and seats become independent one-time charges with their own renewal date, tracked and re-charged individually — which is significant enough (it touches checkout, add/cancel/upgrade, and the webhook, and directly affects real billing correctness) that it's being scoped as its own dedicated batch rather than rushed in alongside everything else here.
- **Parents pasting a different student's essay into their own paid seat's AI feedback**: technically undetectable — the AI can't verify authorship of pasted text. This is a policy problem, not a bug, and the correct tool is the click-through acknowledgment screen Dan asked for — which ships with the parent tier once parent accounts exist to attach it to.
